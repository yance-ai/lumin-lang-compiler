...
Value lumin_del(Value* arr, Value idx);    // del(arr, idx)：原地删除，返回自身
Value lumin_insert(Value* arr, Value idx, Value val);  // insert(arr, idx, val)：原地插入
...
Value lumin_array_add(Value* arr, Value val);  // 原地追加（2x扩容），返回自身
...
Value lumin_array_clear(Value* v);                // 原地清空（保留capacity），返回自身
