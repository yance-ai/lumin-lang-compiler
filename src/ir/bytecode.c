    ...
    for(int i = 0; i < fn->code_len; i++) {
        ...
        switch(in.op) {
            ...
            case OPC_ARRAY_LIT:
                ...
                break;
            case OPC_BUILTIN: {
                static const char* bname[] = {"len", "type", "input", "range", "substr", "toupper", "tolower", "split", "del", "insert", "floor", "ceil", "abs", "sqrt", "max", "min", "join", "contains", "repeat", "replace", "sum", "avg"};
                const char* bn = (in.a >= 0 && in.a < 22) ? bname[in.a] : "?";
