...
static void c_expr(Ctx* c, AstNode* node)
{
    ...
    switch(node->type) {
        ...
        case AST_CALL: {
            ...
            static const char* bnames[BUILTIN_COUNT] = {"len", "type", "input", "range", "substr", "toupper", "tolower", "split", "del", "insert", "floor", "ceil", "abs", "sqrt", "max", "min", "join", "contains", "repeat", "replace", "sum", "avg"};
            int bid = -1;
